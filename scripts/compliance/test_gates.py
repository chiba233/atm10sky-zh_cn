...
def _c15(mods):
    ...
    d = (mods.parent.parent / 'resourcepacks' / 'ATM10Sky汉化包'
...
def _c9(mods):
    ...
    p = (mods.parent.parent / 'resourcepacks' / 'ATM10Sky汉化包' /
...
def _relics_lang(root, extra):
    ...
    输出里照样带规则 id，反例会假绿。"""
    p = (root / 'resourcepacks' / 'ATM10Sky汉化包'
...
def _ijp_fixture(tmp, tiers, keys, make_config=True):
    ...
    lang = (tmp / 'ijptree' / 'resourcepacks' / 'ATM10Sky汉化包'
...
def _bee_fixture(tmp, en_quest, zh_quest, en_names=None, zh_names=None):
    ...
    (zq / 'zz_hanhua_c.snbt').write_text('{\n\tquest.AAA.quest_desc: "%s"\n}\n' % zh_quest,
                                         encoding='utf-8')
    zl = tree / 'resourcepacks' / 'ATM10Sky汉化包' / 'assets' / 'productivebees' / 'lang'
...
def _probe_fixture(tmp, consts, lang, ns_dir=None, drop_lang=False):
    ...
    (d / 'hanhua_pack_check.js').write_text(
        ...
        + '})()\n', encoding='utf-8')
    if not drop_lang:
        ld = (tree / 'resourcepacks' / 'ATM10Sky汉化包' / 'assets'
...
_PROBE_OK = {'PROBE_KEY': 'atm10skyzhcn.pack.version',
             'PROBE_NAMESPACE': 'atm10skyzhcn',
             'PACK_VERSION': 'r19'}


@missing_case('探针键名与值都跟脚本对得上 → 必须绿（证明这道闸不是一律红）')
def _m33(tmp, tree):
    t = _probe_fixture(tmp, _PROBE_OK, {'atm10skyzhcn.pack.version': 'r19'})
    rc, out = _probe_run(tmp, t)
    return rc == 0 and 'atm10skyzhcn.pack.version' in out
...
def _m35(tmp, tree):
    t = _probe_fixture(tmp, _PROBE_OK, {'atm10skyzhcn.pack.ver': 'r19'})
    rc, out = _probe_run(tmp, t)
...
def _m36(tmp, tree):
    t = _probe_fixture(tmp, _PROBE_OK, {'atm10skyzhcn.pack.version': 'r18'})
    rc, out = _probe_run(tmp, t)
...
def _m37(tmp, tree):
    ...
    t = _probe_fixture(tmp, consts, {'atm10skyzhcn.pack.version': '@@PATCHVER@@'})
    rc, out = _probe_run(tmp, t)
...
# 长得一模一样。口子只有一个：versions/<版本>/quest_untranslated.json 逐条登记。
# 口子开了就得有反例证明它没被开大——没写理由、登记了上游没有的键、登记了已经译了
# 的键、同一个键登记两遍、整张表不见、不传版本，六种写歪的登记都必须红，否则这张表
...
@missing_case('上游新加的键既没译也没登记 → 必须红')
def _m58(tmp, tree):
