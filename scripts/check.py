...
PACK_DIR = TREE / 'resourcepacks' / 'ATM10Sky汉化包'
...
def _xml_parses(rule):
    ...
    if not hit:
        ...
        base = TREE / 'resourcepacks' / 'ATM10Sky汉化包' / 'assets'
...
def _js_no_const_inside_block(rule):
    ...
    函数体 / 回调体的**顶层** const 是安全的：拿整包 ATM10 Sky 的 kubejs 对照过，上游
    28 处缩进 const 全都在 `ServerEvents.recipes(x => {` 这类回调体顶层，没有任何
