...
spaced = tmp / '我的 Game Dir With Spaces 天境'
...
loose = tmp / 'loose' / 'ATM10SKY-hanhua'   # 父目录 tmp/loose 不含 mods/options.txt
...
frel = fresh / 'atm10sky-zh_cn-client'
frel.mkdir()
for d in ('config', 'kubejs', 'mods', 'vaultpatcher'):
...
#   resourcePacks:[...]\r,"file/ATM10Sky汉化包-7.2.zip"]
...
FAKE_RELEASE = ('{\n  "tag_name": "vr99",\n  "assets": [\n    ' + ',\n    '.join(
    [_asset('atm10sky-zh_cn-client-r99-sky%s.zip' % v) for v in ('7.0', '7.1', '7.2')]
    + [_asset('atm10sky-zh_cn-server-r99-sky%s.zip' % v) for v in ('7.0', '7.1', '7.2')]
) + '\n  ]\n}\n')
...
if not IS_WIN:                       # install.sh 只跑在 macOS / Linux
    for ver in ('7.0', '7.1', '7.2'):
        ...
        want = 'atm10sky-zh_cn-client-r99-sky%s.zip' % ver
    ...
    only_72 = '{"tag_name": "vr99", "assets": [%s]}' % _asset('atm10sky-zh_cn-client-r99-sky2.0.4.zip')
...
if not IS_WIN:                       # install.sh 只跑在 macOS / Linux
    ...
    def make_case(tag, break_digest=False):
        ...
        def payload(d, mark):
            ...
            for s_ in ('install.sh', 'install.ps1'):
                ...
                if mark == 'NEW':      # 水印：验证原入口的脚本确实被换成了新版
                    t += '\n# ATM10SKY-TEST-NEW-INSTALLER\n'
                (d / s_).write_text(t, encoding='utf-8')

        srcd = instd / 'atm10sky-zh_cn-client'
        srcd.mkdir()
        payload(srcd, 'OLD')
        newd = root / 'newpkg' / 'atm10sky-zh_cn-client'
        newd.mkdir(parents=True)
        payload(newd, 'NEW')

        zname = f'atm10sky-zh_cn-client-r99-sky{MCVER}.zip'
        zpath = root / zname
        with zipfile.ZipFile(zpath, 'w', zipfile.ZIP_DEFLATED) as z:
            for q in newd.rglob('*'):
                if q.is_file():
                    z.write(q, ('atm10sky-zh_cn-client/' + q.relative_to(newd).as_posix()))
        ...
        SERVED[f'/repos/chiba233/atm10sky-zh_cn/releases/latest'] = ('application/json',
                                                                  rel_json.encode())
    ...
    assert 'ATM10SKY-TEST-NEW-INSTALLER' in (srcd / 'install.sh').read_text(encoding='utf-8'), \
        f'原入口的 install.sh 没被换成新版：\n{out}'
    assert any((srcd / 'backups').glob('*')), f'新版安装器的备份没归并回原入口：\n{out}'
    assert list(instd.glob('.atm10sky-hanhua-update-*')), f'没留下新版安装器目录：\n{out}'
    ...
    assert 'ATM10SKY-TEST-NEW-INSTALLER' not in (srcd2 / 'install.sh').read_text(encoding='utf-8'), \
        f'摘要不符却已经换掉了原入口的安装器：\n{out2}'
    ...
    assert not list(instd3.glob('.atm10sky-hanhua-update-*')), f'关了更新检查却还是下载了：\n{out3}'
