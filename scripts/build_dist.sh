...
#   dist/atm10sky-zh_cn-client-v<版本>.zip
#   dist/atm10sky-zh_cn-server-v<版本>.zip
...
CBASE="atm10sky-zh_cn-client"
SBASE="atm10sky-zh_cn-server"
...
if [ ! -d "$UPROOT/kubejs" ]; then
  echo "  取 ATM10 Sky ${MC} 的官方文件（只要 overrides，不下 jar）"
...
PACK_SRC="$TREE/resourcepacks/ATM10Sky汉化包"
PACK_NAME="ATM10Sky汉化包-${MC}"
echo "───── 构建 整合包 ${MC} ─────"
...
printf '[InternetShortcut]\r\nURL=https://github.com/chiba233/atm10sky-zh_cn\r\n' > "$CSTAGE/项目主页与反馈.url"
...
CZIP="dist/${CBASE}-${VERSION}-sky${MC}.zip"
SZIP="dist/${SBASE}-${VERSION}-sky${MC}.zip"
rm -f "$CZIP" "$SZIP"
...
python3 scripts/compliance/verify_dist.py dist/*-${VERSION}-sky*.zip
echo
echo "全部完成：$(ls dist/*-${VERSION}-sky*.zip | wc -l | tr -d " ") 个包"
ls -1 dist/*-${VERSION}-sky*.zip
