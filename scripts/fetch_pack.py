...
"""把某个版本的 ATM10 Sky 整合包备齐成一个能当 ATM_PACK_ROOT 用的目录。
...
PROJECT = 1298402         # CurseForge 上的 All the Mods 10: To the Sky
...
def tree_digest(root, only=None):
    ...
    ATM10 Sky 的某个已发布版本，它的 overrides 内容是**不会变**的。把指纹记进仓库，
...
def check_digest(ver, digest, record=False):
    ...
    if want != digest:
        sys.exit('❌ ATM10 Sky %s 的 overrides 内容与仓库记录的指纹对不上\n'
                 '   记录 %s\n   实得 %s\n'
...
def find_file_id(ver):
    ...
    sys.exit('❌ CurseForge 上找不到 ATM10 Sky %s\n   最近 50 个文件里有: %s'
             % (ver, ' '.join(have)))
...
def main(ver, out, jars=True, record=False):
    ...
    print('ATM10 Sky %s → fileID %s' % (ver, fid))
    z = out.parent / ('atm10sky-%s.zip' % ver)
    if not z.exists():
...
if __name__ == '__main__':
    ...
    if '--verify' in sys.argv:
        ...
        # （整合包会用 overrides/mods/ 里的 jar 去盖掉 manifest 里那一份），
        # 那是指纹的正当组成部分。
