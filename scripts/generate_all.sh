...
: "${ATM_PACK_ROOT:=/Users/yumeka/Documents/minecraft/.minecraft/versions/ATM10SKY}"
export ATM_PACK_ROOT
[ -d "$ATM_PACK_ROOT/kubejs" ] || {
  echo "❌ ATM_PACK_ROOT 不像整合包目录（缺 kubejs/）: $ATM_PACK_ROOT"; exit 1; }
# 上面那条只判「像不像整合包」，判不出**是哪个**——ATM10 与 ATM10 Sky 的目录结构
# 一模一样。指错了会拿另一个包的字节生成一整套汉化，而且全程不报错。装好的实例里有
