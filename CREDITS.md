...
- 模块全部在 `src/vaultpatcher/modules/` 里，以 RFTools 系为主。哪些模块出货由构建时按
  本版整合包的字节现判：目标类不在这一版里的不出货，条数过大的也不出货——这张表每次替换
  调用都要线性扫一遍，出货会全局掉帧。
...
本包为此在 `src/books/` 下存「位置 + 原文 → 译文」的映射，构建时套用；原文对不上就当场报错，
不会悄悄拿旧译文盖掉上游改过的正文。
...
的定点映射（`src/upstream/`）。构建时对着目标版本的官方文件套用，上游改了哪一行都会
当场报错，而不是发出一个「旧上游 + 我们的改动」的包把人家的修复覆盖掉。

本包另外注入几个自己的 KubeJS 脚本（`src/kubejs/`）：资源包生效自检与更新提示只在客户端跑，
删掉即完全回退。
...
翻译问题、漏翻、显示异常都请提到 [Issues](https://github.com/chiba233/atm10sky-zh_cn/issues)，
附截图与所在界面。
