...
# ATM10 Sky @@MCVER@@ 汉化补丁安装器 (Windows)
# 版本号一律用 @@MCVER@@ 占位，由 scripts/build_dist.sh 按目标整合包版本填。
# 用法：把整个汉化文件夹放进 ATM10 Sky 实例根目录后，双击「双击安装-Windows.bat」，
# 或在 PowerShell 中运行：
#   .\install.ps1                    # 交互菜单
...
$PackEntry = 'file/ATM10Sky汉化包-@@MCVER@@.zip'
...
# 也有个 mods\（装着 vaultpatcher.jar）。用 jar 数量区分：ATM10 Sky 有 300+ 个，汉化包只有 1 个。
# ── 版本检查 ────────────────────────────────────────────────────────────
# 补丁自己的版本号，由 build_dist.sh 现填。
$script:PatchVer = '@@PATCHVER@@'
$script:PackMcVer = '@@MCVER@@'
$script:Repo     = 'chiba233/atm10sky-zh_cn'
    ...
    try {
        ...
        $r = Invoke-RestMethod -Uri "https://api.github.com/repos/$($script:Repo)/releases/latest" `
                               -Headers @{ 'Accept' = 'application/vnd.github+json'
                                           'User-Agent' = 'atm10sky-zh_cn-installer' } `
                               -TimeoutSec 6
...
function Invoke-OneClickUpdate {
    ...
    # Release 同时带多个 ATM10 Sky 版本和客户端/服务端包；必须按当前包的整合包版本
    # 精确选择客户端 zip，不能只取 assets[0]，否则会把 7.0 用户升级到 7.2 包。
    $suffix = '-sky' + [regex]::Escape($script:PackMcVer) + '\.zip$'
    $asset = @($release.assets | Where-Object {
        $_.name -match '^atm10sky-zh_cn-client-.+' -and $_.name -match $suffix
    }) | Select-Object -First 1
    if (-not $asset) {
        Write-Host "❌ 最新版 $latest 没有 ATM10 Sky $($script:PackMcVer) 的客户端安装包，未做任何改动。"
        return
    }
    ...
    $stage = Join-Path $script:Target ".atm10sky-hanhua-update-$stamp"
    ...
    try {
        ...
        Invoke-WebRequest -Uri $asset.browser_download_url -OutFile $zip -UseBasicParsing `
            -Headers @{ 'User-Agent' = 'atm10sky-zh_cn-installer' } -TimeoutSec 120
        ...
        $next = @(Get-ChildItem -LiteralPath $stage -Recurse -Filter 'install.ps1' -File |
                  Where-Object { $_.Directory.Name -eq 'atm10sky-zh_cn-client' }) | Select-Object -First 1
...
function Check-Target {
    ...
    while ($true) {
        $inp = Read-Host '请输入 ATM10 Sky 实例根目录完整路径（q 退出）'
        $inp = $inp.Trim()
        ...
        Write-Host '❌ 该路径下没找到 ATM10 Sky 的 mods\（应该有几百个 .jar），请重试。'
    }
}
...
# ATM10 Sky @@MCVER@@ 默认启用的资源包，顺序照抄游戏自己写出来的 options.txt。
...
function Clear-LegacyCCHelp {
    ...
    foreach ($f in $txt) {
        if ($script:BK -and (Test-Path -LiteralPath $script:BK)) {
            ...
            # 而 -Path 会把 [ ] 当通配符（实例目录叫 [1.21.1]ATM10SKY 很常见）。
    ...
    foreach ($f in @('config_ui_generated.json', 'catnip_config_ui.json')) {
        ...
        if (Test-Path -LiteralPath $old -PathType Leaf) {
            ...
            if ($script:BK -and (Test-Path -LiteralPath $script:BK)) {
                ...
                # 实例目录叫 [1.21.1]ATM10SKY 这种在 PCL/HMCL 下很常见。
                [void][System.IO.Directory]::CreateDirectory($to)
...
switch ($Action) {
    ...
    default {
        Write-Host '══════════════════════════════════════════'
        Write-Host ' ATM10 Sky @@MCVER@@ 汉化补丁 — 安装器'
        Write-Host " 目标实例: $script:Target"
        Write-Host '══════════════════════════════════════════'
