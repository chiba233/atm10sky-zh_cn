...
PACK_ENTRY='file/ATM10Sky汉化包-@@MCVER@@.zip'
...
pick_client_asset() {
  ...
  name="$(printf '%s' "$flat" \
    | grep -o '"atm10sky-zh_cn-client-[^"]*-sky@@MCVER@@\.zip"' | head -1 | tr -d '"')"
  [ -n "$name" ] || return 1
  after="${flat#*\"$name\"}"
  url="$(printf '%s' "$after" \
...
do_update() {
  ...
  if [ -z "$asset" ]; then
    say "❌ 最新版 $LATEST_TAG 没有 ATM10 Sky @@MCVER@@ 的客户端安装包（或缺少 SHA-256 摘要），未做任何改动。"
    return 0
  fi
  ...
  stage="$TARGET/.atm10sky-hanhua-update-$stamp"
  zipf="$stage/$name"
  ...
  ok=1
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL --max-time 300 -H 'User-Agent: atm10sky-zh_cn-installer' -o "$zipf" "$url" || ok=0
  else
  ...
  unzip -q -o "$zipf" -d "$stage" || {
    ...
    say "   新版安装器尚未启动；文件保留在：$stage"
    return 0; }
  newdir="$(find "$stage" -type d -name 'atm10sky-zh_cn-client' 2>/dev/null | head -1 || true)"
...
check_target() {
  # 一键更新时，新版安装器是从 <实例>/.atm10sky-hanhua-update-*/ 里被调起来的，
  ...
  if [ -t 0 ]; then
    while :; do
      printf '请输入 ATM10 Sky 实例根目录完整路径（q 退出）: '
      read -r inp || exit 1
      [ "$inp" = "q" ] && exit 1
      ...
      say "❌ 该路径下没找到 ATM10 Sky 的 mods/（应该有几百个 .jar），请重试。"
    done
  fi
...
do_backup() {
  ...
  # 目录名精确到秒。同一秒里装第二次（脚本连跑、或玩家手快）会撞名，而备份是
...
patch_options() {
  ...
  # （不要指望 config/defaultoptions —— ATM10 Sky 并没有装 DefaultOptions 模组，
